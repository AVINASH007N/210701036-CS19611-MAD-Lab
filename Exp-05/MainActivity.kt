...
DBContract.TABLE_STUDENT, values, "${DBContract.COLUMN_REGISTER_NUMBER} = ?", arrayOf(registerNumber.toString())
)
if (rowsAffected > 0) {
...
DBContract.COLUMN_REGISTER_NUMBER, DBContract.COLUMN_NAME, DBContract.COLUMN_CGPA
)
val sortOrder = "${DBContract.COLUMN_REGISTER_NUMBER} ASC" val cursor: Cursor = db.query(
DBContract.TABLE_STUDENT, projection, null, null, null, null, sortOrder
)
val stringBuilder = StringBuilder()
with(cursor) {
while (moveToNext()) {
val registerNumber =
getInt(getColumnIndexOrThrow(DBContract.COLUMN_REGISTER_NUMBER))
val name =
getString(getColumnIndexOrThrow(DBContract.COLUMN_NAME))
val cgpa =
getDouble(getColumnIndexOrThrow(DBContract.COLUMN_CGPA))
stringBuilder.append("Register Number: $registerNumber, Name:
$name, CGPA: $cgpa\n")
}
}
cursor.close()
...
DBContract.TABLE_STUDENT, "${DBContract.COLUMN_REGISTER_NUMBER} = ?", arrayOf(registerNumber.toString())
)
if (rowsDeleted > 0) {
